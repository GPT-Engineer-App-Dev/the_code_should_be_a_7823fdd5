              ...
              {message.text &&
                message.text.split("\n").map((line, index) => {
                  if (line.startsWith("```") && line.endsWith("```")) {
                    return <Code key={index} p={2} bg={message.role === "user" ? "blue.50" : "green.50"} borderRadius="md" children={line.slice(3, -3)} />;
                  } else {
                    return (
                      <Text key={index} p={2} bg={message.role === "user" ? "blue.100" : "green.100"} borderRadius="md">
                        {line}
                      </Text>
                    );
                  }
